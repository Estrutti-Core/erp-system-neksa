...
@section('title', 'Detalhes do Contas a Receber')

@section('topbar-actions')
    <a href="{{ route('receivables.index') }}" class="btn btn-secondary">
        <x-heroicon-o-arrow-left class="w-4 h-4"/> Voltar
    </a>
    <a href="{{ route('receivables.print', $receivable) }}" target="_blank" class="btn btn-secondary">
        <x-heroicon-o-printer class="w-4 h-4"/> Imprimir
    </a>
    @if($receivable->status !== \App\Enums\PaymentStatus::Cancelled)
        <form method="POST" action="{{ route('receivables.cancel', $receivable) }}" onsubmit="return confirm('Deseja realmente cancelar este título?')">
            @csrf
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card">
            <div class="card-header flex justify-between items-center">
                <h2 class="card-title">Título {{ $receivable->code }}</h2>
                @php
                    $colorClass = match($receivable->status) {
            ...
            <div class="card-body">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if($receivable->notes)
                        <div class="md:col-span-2">
                            ...
                            <p class="text-sm text-gray-600 bg-gray-50 p-2 rounded border border-gray-100">{{ $receivable->notes }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
                    <div>
                        ...
                        <span class="text-lg font-semibold">R$ {{ number_format($receivable->total_amount, 2, ',', '.') }}</span>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500 block">Descontos</span>
                        <span class="text-lg font-semibold text-green-600">R$ {{ number_format($receivable->discount_amount, 2, ',', '.') }}</span>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500 block">Juros / Multas</span>
                        <span class="text-lg font-semibold text-amber-600">R$ {{ number_format($receivable->interest_amount, 2, ',', '.') }}</span>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500 block">Valor Líquido</span>
                        <span class="text-lg font-bold text-indigo-700">R$ {{ number_format($receivable->net_amount, 2, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            @foreach($receivable->installments as $inst)
                                <tr>
                                    ...
                                    <td style="text-align:right">
                                        @if($inst->status === \App\Enums\InstallmentStatus::Pending && $receivable->status !== \App\Enums\PaymentStatus::Cancelled)
            ...
            <div class="card-body">
                <div class="flow-root">
                    <ul role="list" class="-mb-8">
                        @forelse($receivable->events as $event)
                            <li>
...
<script>
    ...
    function openPaymentModal(installmentId, amount) {
        ...
        form.action = `{{ route('receivables.pay', ['receivable' => $receivable->id, 'installment' => ':id']) }}`.replace(':id', installmentId);
