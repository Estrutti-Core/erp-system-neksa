...
@section('topbar-actions')
    <a href="{{ route('receivables.export.xlsx', request()->all()) }}" class="btn btn-success flex items-center gap-2" style="background-color: #16a34a; border-color: #16a34a; color: white; border-radius: 8px;">
        <x-heroicon-o-document-arrow-down class="w-4 h-4"/> Exportar (XLSX)
    </a>
    <a href="{{ route('receivables.create') }}" class="btn btn-primary" style="background: linear-gradient(135deg, #4f46e5 0%, #3730a3 100%); border-radius: 8px;">
...
<div style="max-width: 1200px; margin: 0 auto; padding-bottom: 60px;">
    ...
    <div class="card mb-4 shadow-sm" style="border-radius: 12px; padding: 16px; border: 1px solid #e2e8f0;">
        <form method="GET" action="{{ route('receivables.index') }}" id="filter-form" class="flex flex-col gap-3">
                ...
                <tbody>
                    @forelse($receivables as $rec)
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            <td style="padding: 12px 14px;"><strong>{{ $rec->code }}</strong></td>
                            <td>{{ $rec->client->name ?? 'Cliente Avulso' }}</td>
                            <td>{{ \Carbon\Carbon::parse($rec->competence_date)->format('d/m/Y') }}</td>
                            <td style="font-family: monospace; font-weight: 600;">R$ {{ number_format($rec->total_amount, 2, ',', '.') }}</td>
                            <td style="font-family: monospace; font-weight: 600; color: #10b981;">R$ {{ number_format($rec->net_amount, 2, ',', '.') }}</td>
                            <td>
                                @php
                            ...
                            <td style="text-align:right; padding-right: 14px;">
                                <a href="{{ route('receivables.show', $rec) }}" class="btn btn-secondary btn-sm" style="padding: 6px; border-radius: 6px;" title="Ver Detalhes">
