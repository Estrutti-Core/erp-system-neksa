...
<div style="max-width: 900px; margin: 0 auto; padding-bottom: 40px;">
    ...
    <div class="flex items-center justify-between mb-4">
        <a href="{{ route('clients.show', $client) }}" class="btn btn-secondary" style="border-radius: 8px;">
            <x-heroicon-o-arrow-left class="w-4 h-4"/> Voltar
        </a>
        <h2 style="font-size: 18px; font-weight: 700; color: #0f172a;">Editar Cliente: {{ $client->name }}</h2>
    </div>
    ...
    <form method="POST" action="{{ route('clients.update', $client) }}" id="client-form">
        ...
        <div class="card mb-4 shadow-sm" style="border-radius: 12px; border: 1px solid #e2e8f0; padding: 24px;">
            ...
            <div class="grid-2 mb-3">
                <div class="form-group">
                    ...
                    <select id="document_type" name="document_type" class="form-control" onchange="togglePjFields(this.value)" required>
                        <option value="cpf" {{ old('document_type', $client->document_type) == 'cpf' ? 'selected' : '' }}>Pessoa Física (CPF)</option>
                        <option value="cnpj" {{ old('document_type', $client->document_type) == 'cnpj' ? 'selected' : '' }}>Pessoa Jurídica (CNPJ)</option>
                    </select>
                </div>

                <div class="form-group">
                    <label class="form-label" for="document">CPF / CNPJ <span style="color:#ef4444">*</span></label>
                    <input type="text" id="document" name="document" value="{{ old('document', $client->document) }}" class="form-control" placeholder="Digite o documento" required>
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $client->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Ex: João da Silva ou Neksa Tecnologia" required>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $client->phone) }}" class="form-control phone-mask" placeholder="(00) 00000-0000">
                </div>
                <div class="form-group">
                    <label class="form-label" for="phone_secondary">Telefone Secundário</label>
                    <input type="text" id="phone_secondary" name="phone_secondary" value="{{ old('phone_secondary', $client->phone_secondary) }}" class="form-control phone-mask" placeholder="Opcional">
                </div>
            </div>

            <div class="form-group">
                <label class="form-label" for="email">E-mail de Cobrança / Contato</label>
                <input type="email" id="email" name="email" value="{{ old('email', $client->email) }}" class="form-control @error('email') is-invalid @enderror" placeholder="email@exemplo.com">
                @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label class="form-label" for="notes">Observações Internas (Restrições/Mural)</label>
                <textarea id="notes" name="notes" rows="2" class="form-control" placeholder="Notas operacionais internas...">{{ old('notes', $client->notes) }}</textarea>
            </div>
        </div>
        ...
        <div class="card mb-4 shadow-sm" id="pj-info-section" style="border-radius: 12px; border: 1px solid #e2e8f0; padding: 24px; display: none;">
            ...
            <div class="grid-2 mb-3">
                <div class="form-group">
                    ...
                    <input type="text" id="social_name" name="social_name" value="{{ old('social_name', $client->social_name) }}" class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="trade_name">Nome Fantasia da Receita</label>
                    <input type="text" id="trade_name" name="trade_name" value="{{ old('trade_name', $client->trade_name) }}" class="form-control">
                </div>
            </div>
            ...
            <div class="grid-3 mb-3">
                <div class="form-group">
                    ...
                    <input type="text" id="sector" name="sector" value="{{ old('sector', $client->sector) }}" class="form-control" placeholder="Ex: Tecnologia">
                </div>
                <div class="form-group">
                    <label class="form-label" for="opening_date">Data de Abertura</label>
                    <input type="date" id="opening_date" name="opening_date" value="{{ old('opening_date', $client->opening_date ? $client->opening_date->format('Y-m-d') : '') }}" class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="capital_social">Capital Social (R$)</label>
                    <input type="text" id="capital_social" name="capital_social" value="{{ old('capital_social', $client->capital_social ? number_format($client->capital_social, 2, ',', '') : '') }}" class="form-control money" placeholder="0,00">
                </div>
            </div>
            ...
            <div class="grid-3 mb-4">
                <div class="form-group">
                    ...
                    <input type="text" id="company_size" name="company_size" value="{{ old('company_size', $client->company_size) }}" class="form-control" placeholder="Ex: ME, EPP">
                </div>
                <div class="form-group">
                    <label class="form-label" for="legal_nature">Natureza Jurídica</label>
                    <input type="text" id="legal_nature" name="legal_nature" value="{{ old('legal_nature', $client->legal_nature) }}" class="form-control" placeholder="Ex: LTDA">
                </div>
                <div class="form-group">
                    <label class="form-label" for="registration_status">Situação Cadastral</label>
                    <input type="text" id="registration_status" name="registration_status" value="{{ old('registration_status', $client->registration_status) }}" class="form-control" placeholder="Ex: ATIVA">
                </div>
            </div>
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                ...
                <div class="grid-3" style="grid-template-columns: 1fr 2fr; gap: 12px;">
                    <div class="form-group" style="margin-bottom: 0;">
                        ...
                        <input type="text" id="main_cnae_code" name="main_cnae_code" value="{{ old('main_cnae_code', $primaryCnae?->code) }}" class="form-control" placeholder="Ex: 6202300" style="font-family: monospace;">
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="form-label" style="font-size: 11px;">Descrição da Atividade</label>
                        <input type="text" id="main_cnae_description" name="main_cnae_description" value="{{ old('main_cnae_description', $primaryCnae?->description) }}" class="form-control" placeholder="Descrição do CNAE">
                    </div>
                </div>
            </div>
        ...
        @php $addr = $client->primaryAddress ?? $client->addresses->first(); @endphp
        <div class="card mb-4 shadow-sm" style="border-radius: 12px; border: 1px solid #e2e8f0; padding: 24px;">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; border-bottom: 1px solid #f1f5f9; padding-bottom: 12px;">
            ...
            <div class="grid-2 mb-3">
                <div class="form-group">
                    ...
                    <input type="text" id="zip_code" name="zip_code" value="{{ old('zip_code', $addr->zip_code) }}" class="form-control cep-mask" placeholder="00000-000">
                </div>
                <div></div>
            </div>
            ...
            <div class="grid-2 mb-3">
                <div class="form-group">
                    ...
                    <input type="text" id="street" name="street" value="{{ old('street', $addr->street) }}" class="form-control @error('street') is-invalid @enderror" placeholder="Rua, Avenida..." required>
                    @error('street')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label" for="number">Número</label>
                    <input type="text" id="number" name="number" value="{{ old('number', $addr->number) }}" class="form-control" placeholder="123, S/N">
                </div>
            </div>
            ...
            <div class="grid-2 mb-3">
                <div class="form-group">
                    ...
                    <input type="text" id="complement" name="complement" value="{{ old('complement', $addr->complement) }}" class="form-control" placeholder="Apto, Sala, Bloco...">
                </div>
                <div class="form-group">
                    <label class="form-label" for="neighborhood">Bairro</label>
                    <input type="text" id="neighborhood" name="neighborhood" value="{{ old('neighborhood', $addr->neighborhood) }}" class="form-control">
                </div>
            </div>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="city" name="city" value="{{ old('city', $addr->city) }}" class="form-control @error('city') is-invalid @enderror" required>
                ...
                <div class="form-group">
                    ...
                    <select id="state" name="state" class="form-control @error('state') is-invalid @enderror" required>
                        ...
                        @foreach(['AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'] as $uf)
                            <option value="{{ $uf }}" {{ old('state', $addr->state) == $uf ? 'selected' : '' }}>{{ $uf }}</option>
                        @endforeach
                    </select>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('clients.show', $client) }}" class="btn btn-secondary" style="border-radius: 8px;">Cancelar</a>
...
<script>
    ...
    function addEquipmentRow(equipment = {}) {
        ...
        card.innerHTML = `
            <input type="hidden" name="equipments[${equipmentIndex}][id]" value="${equipment.id || ''}">
    ...
    function addContactRow(contact = {}) {
        ...
        card.innerHTML = `
            <input type="hidden" name="contacts[${contactIndex}][id]" value="${contact.id || ''}">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Load existing CNAEs
        @foreach($client->cnaes->where('pivot.is_primary', false) as $cnae)
            addSecondaryCnaeRow("{{ $cnae->code }}", "{!! addslashes($cnae->description) !!}");
        @endforeach

        // Load existing contacts
        @if($client->contacts->isEmpty())
            addContactRow({ name: '', is_primary: true });
        @else
            @foreach($client->contacts as $contact)
                addContactRow({
                    ...
                    is_phone_blocked: {{ $contact->is_phone_blocked ? 'true' : 'false' }},
                    is_whatsapp_blocked: {{ $contact->is_whatsapp_blocked ? 'true' : 'false' }},
                    is_email_blocked: {{ $contact->is_email_blocked ? 'true' : 'false' }}
                });
            @endforeach
        @endif
        ...
        @foreach($client->equipments as $equipment)
            addEquipmentRow({
                ...
                brand: "{!! addslashes($equipment->brand) !!}",
                model: "{!! addslashes($equipment->model) !!}",
                serial_number: "{!! addslashes($equipment->serial_number) !!}",
                notes: "{!! addslashes(preg_replace('/\s+/', ' ', $equipment->notes)) !!}"
            });
        @endforeach
