@extends('layouts.app')
@section('title', 'Faturamento da Venda ' . $sale->code)

@section('content')
<div class="card mb-4" style="max-width: 900px; margin: 0 auto;" x-data="invoicingForm()">
    <div class="flex justify-between items-center mb-6 pb-4 border-b">
        <div>
            <h2 class="text-lg font-bold">Faturamento da Venda</h2>
            <p class="text-sm text-muted">Cliente: <strong>{{ $sale->client->name ?? 'Cliente Avulso' }}</strong></p>
        </div>
        <div class="text-right">
            <span class="text-sm text-muted block">Valor Total a Faturar</span>
            <div class="text-2xl font-black text-primary">R$ {{ number_format($sale->total_amount, 2, ',', '.') }}</div>
        </div>
    </div>
    ...
    <form action="{{ route('sales.complete', $sale) }}" method="POST" id="payment-form" @submit="handleSubmit($event)">
        @csrf
        ...
        <div class="flex gap-3 mt-6">
            ...
            <a href="{{ route('sales.show', $sale) }}" class="btn btn-secondary btn-lg" style="padding: 12px 32px;">
                Voltar
            </a>
        </div>
    </form>
