...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <form action="{{ route('payment-conditions.store') }}" method="POST">
        @csrf

        <div class="form-group mb-4">
            <label for="name" class="form-label">Nome da Condição *</label>
            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Ex: Pix à Vista, Cartão 3x, 30/60 dias" required>
            @error('name')
        ...
        <div class="form-group mb-4">
            ...
            <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                <option value="cash" {{ old('type') === 'cash' ? 'selected' : '' }}>À Vista</option>
                <option value="installments" {{ old('type') === 'installments' ? 'selected' : '' }}>Parcelado</option>
        ...
        <div class="flex gap-4">
            <div class="form-group mb-4 flex-1">
                ...
                <input type="number" name="installments_count" id="installments_count" class="form-control @error('installments_count') is-invalid @enderror" value="{{ old('installments_count', 1) }}" min="1" required>
            ...
            <div class="form-group mb-4 flex-1">
                ...
                <input type="number" name="interval_days" id="interval_days" class="form-control @error('interval_days') is-invalid @enderror" value="{{ old('interval_days', 30) }}" min="0" required>
                @error('interval_days')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="form-group mb-4">
            ...
            <select name="default_payment_method" id="default_payment_method" class="form-control @error('default_payment_method') is-invalid @enderror">
                ...
                @foreach($methods as $method)
                    <option value="{{ $method->value }}" {{ old('default_payment_method') === $method->value ? 'selected' : '' }}>{{ $method->label() }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group mb-4">
            ...
            <select name="default_financial_account_id" id="default_financial_account_id" class="form-control @error('default_financial_account_id') is-invalid @enderror">
                ...
                @foreach($accounts as $acc)
                    <option value="{{ $acc->id }}" {{ old('default_financial_account_id') == $acc->id ? 'selected' : '' }}>{{ $acc->name }}</option>
                @endforeach
            </select>
