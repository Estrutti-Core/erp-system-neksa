...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <form method="POST" action="{{ route('settings.statuses.store') }}">
        @csrf
        ...
        <div class="form-group">
            ...
            <select name="color" id="color" class="form-control @error('color') is-invalid @enderror" required>
                <option value="blue" {{ old('color') == 'blue' ? 'selected' : '' }}>Azul (Padrão/Aberto)</option>
                <option value="green" {{ old('color') == 'green' ? 'selected' : '' }}>Verde (Concluído/Sucesso)</option>
                <option value="red" {{ old('color') == 'red' ? 'selected' : '' }}>Vermelho (Cancelado/Erro)</option>
                <option value="amber" {{ old('color') == 'amber' ? 'selected' : '' }}>Âmbar (Atenção/Pendente)</option>
                <option value="slate" {{ old('color') == 'slate' ? 'selected' : '' }}>Cinza/Slate (Neutro)</option>
                <option value="indigo" {{ old('color') == 'indigo' ? 'selected' : '' }}>Índigo (Destaque)</option>
                <option value="purple" {{ old('color') == 'purple' ? 'selected' : '' }}>Roxo</option>
                <option value="pink" {{ old('color') == 'pink' ? 'selected' : '' }}>Rosa</option>
                <option value="emerald" {{ old('color') == 'emerald' ? 'selected' : '' }}>Esmeralda</option>
                <option value="cyan" {{ old('color') == 'cyan' ? 'selected' : '' }}>Ciano</option>
            </select>
            @error('color')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        ...
        <div class="grid-2">
            <div class="form-group">
                ...
                <input type="number" name="expected_time_minutes" id="expected_time_minutes" value="{{ old('expected_time_minutes') }}" min="1" class="form-control @error('expected_time_minutes') is-invalid @enderror" placeholder="Opcional">
            ...
            <div class="form-group">
                ...
                <input type="number" name="max_stay_minutes" id="max_stay_minutes" value="{{ old('max_stay_minutes') }}" min="1" class="form-control @error('max_stay_minutes') is-invalid @enderror" placeholder="Opcional">
        ...
        <div class="form-group" style="margin-top: 10px;">
            ...
            <div style="margin-top: 8px;">
                <label class="flex items-center gap-2" style="font-weight: normal; cursor: pointer;">
                    <input type="checkbox" name="is_open_state" value="1" {{ old('is_open_state') ? 'checked' : '' }}>
            ...
            <div style="margin-top: 8px;">
                <label class="flex items-center gap-2" style="font-weight: normal; cursor: pointer;">
                    <input type="checkbox" name="is_completed_state" value="1" {{ old('is_completed_state') ? 'checked' : '' }}>
            ...
            <div style="margin-top: 8px;">
                <label class="flex items-center gap-2" style="font-weight: normal; cursor: pointer;">
                    <input type="checkbox" name="is_cancelled_state" value="1" {{ old('is_cancelled_state') ? 'checked' : '' }}>
        ...
        <div class="form-group" style="border-top: 1px solid #e2e8f0; padding-top: 15px; margin-top: 20px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 8px;">
                @foreach($statuses as $other)
                    <label class="flex items-center gap-2" style="font-weight: normal; cursor: pointer;">
                        <input type="checkbox" name="allowed_transitions[]" value="{{ $other->id }}" {{ is_array(old('allowed_transitions')) && in_array($other->id, old('allowed_transitions')) ? 'checked' : '' }}>
        ...
        <div style="margin-top: 24px; text-align: right;">
            <button type="submit" class="btn btn-primary" style="padding: 10px 24px;">Salvar Status</button>
        </div>
    </form>
</div>
@endsection
