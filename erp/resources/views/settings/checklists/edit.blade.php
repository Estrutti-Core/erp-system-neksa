...
@section('content')
<div class="max-w-5xl" x-data="checklistBuilder(
    {{ json_encode($checklist->sections->map(fn($s) => [
        'id'          => $s->id,
        'title'       => $s->title,
        ...
        'questions'   => $s->questions->map(fn($q) => [
            ...
            'options_text'   => is_array($q->options_json) ? implode("\n", $q->options_json) : '',
            'source_block_id'=> $q->source_block_id,
        ])->values(),
    ])->values()) }},
    {{ json_encode($blocks->map(fn($b) => [
        'id'   => $b->id,
        'name' => $b->name,
    ...
    <form action="{{ route('settings.checklists.update', $checklist) }}" method="POST" @submit="prepareSubmit">
        @csrf
        @method('PUT')
        ...
        <div class="card mb-4">
            ...
            <div class="form-group">
                ...
                <textarea name="description" rows="2" class="form-control">{{ old('description', $checklist->description) }}</textarea>
            </div>
        </div>
        ...
        <template x-for="(section, si) in sections" :key="section._key">
            <div class="card mb-4" style="border-left: 3px solid #6366f1;">
                ...
                <div class="flex gap-3 items-start mb-3">
                    ...
                    <div style="flex:1;">
                        <input type="hidden" :name="`sections[${si}][order]`" :value="si * 10">
                    ...
                    <div style="padding-top:20px; display:flex; gap:6px;">
                        <button type="button" class="btn btn-secondary btn-sm" @click="moveSection(si,-1)"
                            :disabled="si===0" style="padding:6px 8px;">▲</button>
                        <button type="button" class="btn btn-secondary btn-sm" @click="moveSection(si,1)"
                            :disabled="si===sections.length-1" style="padding:6px 8px;">▼</button>
                        <button type="button" class="btn btn-secondary btn-sm" @click="removeSection(si)"
                ...
                <div class="border-t pt-3">
                    <template x-for="(q, qi) in section.questions" :key="q._key">
                        <div class="question-card mb-2">
                            <input type="hidden" :name="`sections[${si}][questions][${qi}][id]`" :value="q.id || ''">
                            <div class="question-header">
                                ...
                                <div style="width:180px;">
                                    ...
                                    <select :name="`sections[${si}][questions][${qi}][question_type]`"
                                        x-model="q.question_type" class="form-control">
...
<style>
.question-card { background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:12px; }
.question-header { display:flex; gap:10px; align-items:flex-start; flex-wrap:wrap; }
.options-container { padding-top:8px; border-top:1px dashed #e2e8f0; }
</style>

<script>
function checklistBuilder(initialSections, blocks) {
    return {
        blocks: blocks,
        sections: [],
        _sectionKey: 0,
        ...
        init() {
            this.sections = (initialSections || []).map(s => ({
                ...s,
        ...
        addSection() {
            this.sections.push({ _key: this._sectionKey++, id: null, title: '', description: '', order: this.sections.length * 10, questions: [] });
        },
        ...
        moveSection(si, dir) {
            const t = si + dir;
            if (t < 0 || t >= this.sections.length) return;
            [this.sections[si], this.sections[t]] = [this.sections[t], this.sections[si]];
            this.sections = [...this.sections];
        },

        addQuestion(section) {
            section.questions.push({ _key: this._questionKey++, id: null, question_text: '', question_type: 'text', is_required: false, options_text: '', source_block_id: null });
        },

        removeQuestion(section, qi) { section.questions.splice(qi, 1); },

        moveQuestion(section, qi, dir) {
            const t = qi + dir;
            if (t < 0 || t >= section.questions.length) return;
            [section.questions[qi], section.questions[t]] = [section.questions[t], section.questions[qi]];
        ...
        importBlock(section, block) {
            block.questions.forEach(bq => {
                section.questions.push({
                    _key: this._questionKey++, id: null,
